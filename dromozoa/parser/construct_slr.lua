...
return function (prods, start)
  ...
  local followset = make_followset(prods, start_head)
    ...
    for item in items:each() do
      ...
      if symbol == nil then
        if equal(head, start_head) then
          ...
          for symbol in followset[head]:each() do
            insert_action(actions, { i, symbol }, { "reduce", { head, body } })
          end
        end
      elseif prods[symbol] == nil then
        local goto_items = lr0.goto_(prods, items, symbol)
  ...
  for items, i in set_of_items:each() do
    for head in prods:each() do
      local goto_items = lr0.goto_(prods, items, head)
      local j = set_of_items[goto_items]
      if j ~= nil then
